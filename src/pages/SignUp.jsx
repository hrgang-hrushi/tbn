...
export default function SignUp(){
  ...
  const saveUserToFirestore = async (user, name) => {
    ...
    console.log('Saving user to Firestore:', userPath)
    try {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Signup complete, navigating to welcome')
      navigate('/welcome')
    } catch (err) {
  ...
  return (
    <div className="signup-page" style={{ 
      ...
      <h2 style={{ textAlign: 'center', marginBottom: 8, fontSize: 32, fontWeight: 600, color: '#fff' }}>Join TBN</h2>
      <p style={{ textAlign: 'center', color: 'var(--muted)', marginBottom: 32, fontSize: 16 }}>Start your entrepreneurial journey today.</p>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <label style={{ fontSize: 14, fontWeight: 500, color: 'var(--muted)' }}>Full Name</label>
          <input 
            type="text" 
            required 
            value={displayName}
